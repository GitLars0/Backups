...
public class CartModel : PageModel
{
	...
	public async Task<IActionResult> OnPostAsync(string customerName, string building, string roomNumber, string locationNotes){
		var cartId = HttpContext.Session.GetGuid("CartId");
		if (cartId is null){
			return Page();
		}
		await _mediator.Send(new CartCheckout.Request(cartId.Value, building, roomNumber, locationNotes, customerName));
